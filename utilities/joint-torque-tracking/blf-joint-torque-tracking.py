...
logPrefix = "[joint-torque-tracking-application]"
...
class SinusoidTrajectoryGenerator(Trajectory):
    def __init__(
        self,
        min_delta_torque: npt.NDArray[np.float_],
        max_delta_torque: npt.NDArray[np.float_],
        delta_torque_increment: npt.NDArray[np.float_],
        min_frequency: npt.NDArray[np.float_],
        max_frequency: npt.NDArray[np.float_],
        frequency_decrement: npt.NDArray[np.float_],
    ):
        self.min_delta_torque = min_delta_torque
        self.max_delta_torque = max_delta_torque
        self.delta_torque_increment = delta_torque_increment
    ...
    def from_parameter_handler(
        ...
        min_delta_torque = param_handler.get_parameter_vector_float(
            "min_delta_torque"
        )
        max_delta_torque = param_handler.get_parameter_vector_float(
            "max_delta_torque"
        )
        delta_torque_increment = param_handler.get_parameter_vector_float(
            "delta_torque_increment"
        )
        ...
        return SinusoidTrajectoryGenerator(
            min_delta_torque=min_delta_torque,
            max_delta_torque=max_delta_torque,
            delta_torque_increment=delta_torque_increment,
    ...
    def generate(
        ...
        A = self.min_delta_torque[joint_index]
        f_in = self.max_frequency[joint_index]
        f_end = self.min_frequency[joint_index]
        delta_f = -self.frequency_decrement[joint_index]
        delta_torque_increment = self.delta_torque_increment[joint_index]

        # Generate the trajectory
        trajectory = []

        while np.abs(A) <= np.abs(self.max_delta_torque[joint_index]):

            for f in np.arange(f_in, f_end, delta_f):
                t_max = 2.0 / f
                t = np.arange(0, t_max, dt)
                trajectory.extend(initial_torque + A * np.sin(2 * np.pi * f * t))

            A += delta_torque_increment
...
class RampTrajectoryGenerator(Trajectory):
    ...
    def from_parameter_handler(
        ...
        max_torque_increment = param_handler.get_parameter_vector_float(
            "max_torque_increment"
        )
        ...
        return RampTrajectoryGenerator(
            max_torque_increment=max_torque_increment,
            speed_in=speed_in,
            speed_end=speed_end,
    ...
    def generate(
        ...
        max_torque_increment = (
            -self.max_torque_increment[joint_index]
            if opposite_direction
            else self.max_torque_increment[joint_index]
        )
        speed = (
            -self.speed[joint_index] if opposite_direction else self.speed[joint_index]
        )
        t_start = 0.0
        t_end = max_torque_increment / speed
        t = t_start
        motor_torque = initial_torque

        while t <= t_end:
            trajectory.append(motor_torque)
            t = t + dt
            motor_torque = motor_torque + dt * speed
...
def main():
    ...
    arg_parser = argparse.ArgumentParser(
        description="This application enables users to generate trajectories for joint torque."
    )
    ...
    param_file = "joint-torque-tracking-options.ini"
    ...
    bypass_joint_torque_measure = param_handler.get_parameter_vector_bool(
        "bypass_joint_torque_measure"
    )
    if len(bypass_joint_torque_measure) != len(joints_to_control):
        raise ValueError(
            "{} The number of joints must be equal to the size of the bypass_joint_torque_measure parameter".format(
                logPrefix
            )
        )
    ...
    poly_drivers["REMOTE_CONTROL_BOARD"] = build_remote_control_board_driver(
        ...
        local_prefix="joint_torque_tracking",
    )
    ...
    for counter, starting_position in enumerate(starting_positions):
            ...
            if trajectory_type == "ramp":
                ...
                trajectories.append(
                    trajectory_generator.generate(
                        dt=dt.total_seconds(),
                        initial_torque=joint_torques[joint_index]
                        if not (bypass_joint_torque_measure[joint_index])
                        else 0,
        ...
        # reset control modes for torque
        control_modes = [
        ...
        for _ in tqdm(range(trajectories_length.max())):
            ...
            for joint_idx, trajectory in enumerate(trajectories):
                ...
                if traj_index < len(trajectory):
                    if is_out_of_safety_limits[joint_idx]:
                        torque_reference.append(0)
                    else:
                        if isGazebo:
                            torque_reference.append(
                            ...
                            if (
                                np.abs(trajectory[traj_index]) / MotorParameters.k_tau[joints_to_control[joint_idx]]
                            ...
                            torque_reference.append(trajectory[traj_index] 
                                                     + generalizedBiasForcesVector[joint_idx] * compensation_bias_forces_factor[joint_idx]
                                                     )
                else:
            ...
            # merge the position and torque references depending on the control mode
            reference = np.where(
                is_out_of_safety_limits, position_reference, np.array(torque_reference)
            )

            # send the references joint torque (or joint torque for Gazebo)
            if not robot_control.set_references(
            ...
            vectors_collection_server.populate_data(
                "joints::desired::torque", torque_reference
            )
            vectors_collection_server.populate_data(
